...
def _installer_workspace(user):
    ...
    return [
        ...
        NavItem("Host query log", "interop:host_query_list", "search"),
...
def _workspace(user):
    ...
    return [
        ...
        NavItem("Exceptions", "operations:exception_list", "alert-octagon", is_lab or is_clerk),
...
def settings_index(user):
    ...
    items = [
        ...
        SettingsItem("Decision rules", "rules:rule_list", "Clinical rules",
                     "Interpretive comments, flags, reflex additions and automatic release.",
                     is_lab, "rules engine autoverification comment interpretive"),
        SettingsItem("Rule firing log", "rules:execution_list", "Clinical rules",
                     "What the rules did, to which result, and why.", is_lab,
                     "autoverified override audit"),
        ...
        SettingsItem("Disclosure accounting", "compliance:disclosure_list", "Access and privacy",
                     ...
                     "hipaa 164.528"),
        SettingsItem("Data subject requests", "compliance:subject_request_list",
                     "Access and privacy",
                     "Access, erasure, restriction and portability requests, with the "
                     "statutory clock on each.", is_manager,
                     "gdpr article 15 17 18 20 erasure export"),
        SettingsItem("API clients", "integrations:client_list", "Access and privacy",
                     "Machine credentials for the REST API, their scopes and expiry.",
                     is_admin, "token bearer rest integration", system=True),
        SettingsItem("Webhooks", "integrations:webhook_list", "Access and privacy",
                     "Signed event callbacks to external systems.", is_admin,
                     "events callback subscribe", system=True),
        ...
        SettingsItem("Host query log", "interop:host_query_list", "System",
                     "What analysers asked for, and what they were told.", is_manager,
                     "bidirectional query worklist", system=True),
        SettingsItem("ICD-10 codes", "interop:icd10_list", "Test catalogue",
                     "Diagnosis codes attached to orders.", is_manager,
                     "diagnosis indication medical necessity coding"),
        SettingsItem("Exception queue", "operations:exception_list", "Operations",
                     "Everything in the laboratory that needs a person.", is_lab,
                     "problems failures backlog"),
